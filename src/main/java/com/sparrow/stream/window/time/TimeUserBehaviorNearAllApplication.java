...
public class TimeUserBehaviorNearAllApplication {
    private static Logger logger = LoggerFactory.getLogger(TimeUserBehaviorNearAllApplication.class);
    ...
    public static void main(String[] args) throws IOException {
        ...
        env.addSource(new RocketMQSource(new SimpleKeyValueDeserializationSchema(), consumerProps))
                ...
                .apply(new WindowFunction<UserBehaviorBO, UserBehaviorBO, Integer, Window>() {
                    @Override
                    public void apply(Integer integer, Window window, Iterable<UserBehaviorBO> input, Collector<UserBehaviorBO> out) throws Exception {
                        int i = 0;
                        for (UserBehaviorBO u : input) {
                            i++;
                        }
                        System.out.println("apply" + window.toString() + "- count=" + i);
                    }
